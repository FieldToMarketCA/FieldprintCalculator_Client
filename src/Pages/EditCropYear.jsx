  ...
  useEffect(() => {
    async function getCropYear() {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/farms/${FARM_ID}/fields/${FIELD_ID}/cropyears/${cropyearId}`,
        {
          headers: {
  ...
  if (fieldOperationsData.cultivations !== null) {
    for (var i = 0; i < fieldOperationsData.cultivations.length; i++) {
      ...
      if (
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/farms/${farmContext.state._id.$oid}/machines/${fieldOperationsData.cultivations[i].machineId}`,
          {
            headers: {
              token: "Bearer " + user.token,
              "Content-Type": "application/json",
